...
const RegistrationForm = ({ hide, toggleForm }) => {
  const {
    formData,
    handleChange,
    handleSubmit,
  ...
  const handleClose = () => {
    // Llama a toggleForm() después de un registro exitoso para ocultar el formulario
    toggleForm();
  };


  return (
    <div className={`w-full md:w-2/3 lg:w-1/2 xl:w-1/3 h-2/3 bg-gray-900 rounded-3xl p-8 fixed top-1/2 right-1/5 md:right-1/4 xl:left-1/2 transform -translate-y-1/3 z-10 ${hide ? 'hidden' : ''}`}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
         ...
         {/* Botón para cerrar el formulario (visible en dispositivos móviles) */}
         {!hide && (
          <button
            type="button"
            className="text-white mt-4 underline cursor-pointer md:hidden"
            onClick={handleClose}
          >
            ¿Iniciaste sesión?
          </button>
        )}
        {/* Nuevo botón para cerrar el formulario (visible en pantallas grandes) */}
        {!hide && (
          <button
            type="button"
            className="text-white mt-4 underline cursor-pointer hidden md:inline lg:inline xl:inline"
            onClick={handleClose}
          >
            Cerrar registro
          </button>
