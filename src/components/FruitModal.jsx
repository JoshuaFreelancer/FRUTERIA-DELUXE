...
export const FruitModal = ({ fruit, onClose, onEdit, onDelete }) => {
  ...
  const handleClose = () => {
    playSound("ui-click"); // <--- Sonido suave al cerrar
    onClose();
  };

  return (
    // Overlay (Fondo oscuro borroso)
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-fruit-wood/30 backdrop-blur-sm transition-all animate-fadeIn">
      {/* Contenedor del Modal */}
      <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden relative animate-scaleIn">
        {/* Botón Cerrar */}
        <button
          onClick={handleClose} // Usamos la función con sonido
        ...
        <div className="flex flex-col md:flex-row h-full">
          {/* Lado Izquierdo: Visual (Imagen Gigante) */}
          <div
          ...
          <div className="w-full md:w-3/5 p-8 flex flex-col justify-between">
            ...
            <div className="flex gap-3 pt-4 border-t border-gray-100">
              <button
                onClick={() => {
                  playSound("ui-click"); // <--- Sonido al hacer clic
                  onEdit(fruit);
                }}
